<template>
  ...
  >
        ...
        <tr v-for="i in numPaths" :key="i - 1">
          ...
          <td>
            <span class="d-flex align-center">
              ...
              ></span>
              {{ pathColorNames[i-1] || '---' }}
              <span class="grey--text ml-1 caption" v-if="pathBrands[i-1]">· {{ pathBrands[i-1] }}</span>
            </span>
          </td>
          <td>
    ...
    <!-- Footer -->
    <div class="sa-footer mt-2 caption grey--text" v-if="status">
      Selector: {{ selectorStr }} &nbsp;|&nbsp; Drive: {{ status.servo_engaged ? 'ENGAGED' : 'neutral' }}
    </div>

    <!-- Load wizard dialog (same structure as Mainsail but uses Fluidd dialog component) -->
    <v-dialog v-model="dialog" max-width="600" scrollable>
      <v-card>
        <v-card-title class="headline">Load T{{ wizard.path }}</v-card-title>
        <v-card-text>
          <!-- Step 1: Brand -->
          <div v-if="step === 1">
            <div class="subtitle-2 mb-2">Select Brand</div>
          ...
          <!-- Step 2: Material -->
          <div v-if="step === 2">
            <div class="subtitle-2 mb-2">Select Material</div>
            <div class="d-flex flex-wrap">
              <v-btn
          ...
          <!-- Step 3: Product line -->
          <div v-if="step === 3">
            <div class="subtitle-2 mb-2">Select Product Line</div>
            <v-list dense>
              <v-list-item
          ...
          <!-- Step 4: Color -->
          <div v-if="step === 4">
            <div class="subtitle-2 mb-2">Select Color</div>
            ...
            <div v-if="wizard.colorHex" class="d-flex align-center mt-3">
              <div class="sa-swatch-lg mr-3" :style="{ background: wizard.colorHex }"></div>
              <div>
                <div class="font-weight-bold">{{ wizard.colorName }}</div>
                <div class="caption grey--text">
                  {{ wizard.colorHex }} · Load: {{ wizard.loadTemp }}°C · Purge: {{ wizard.purgeSpeed }}mm/s
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    stateClass(s) {
      return { loaded: 'green--text', empty: 'grey--text', partial: 'orange--text', unknown: 'amber--text' }[s] ?? ''
    },
...
<style scoped>
.sa-table th { font-size: 11px; text-transform: uppercase; color: #888; }
.sa-swatch {
...
.sa-footer { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 4px; }
</style>
