...
| `autoloader/leds.cfg` | Toolhead Voron logo + nozzle LED status macros. State-driven (PARKED / UNLOADED / ACTIVE / LOADING / ERROR), reads filament color from `printer["autoloader"].path_color_hexes`. Triggered from toolchanger hooks and autoloader load/unload completion. Includes v5-calibrated perceptual color correction (`_sa_set_logo_filament`) — on this build's WS2812B/SK6812 strip the green LED is ~1.5-2x brighter than red/blue, so the macro attenuates green by 45% when green isn't the dominant channel. Reference palette of 17 calibrated hues is documented in the file header. The `_SA_LED_TEST_CASELIGHT` macro is the calibration tool; not used in normal operation but kept for re-verification if hardware ever changes. |
