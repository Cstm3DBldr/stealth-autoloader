...
if [ "$1" = "--uninstall" ]; then
    ...
    rm -f "${CONFIG_DIR}/stealth-autoloader.cfg" 2>/dev/null || true
    rm -f "${HOME}/.moonraker/config/update_manager/stealth-autoloader.ini" 2>/dev/null || true
    echo "[UNINSTALL] Complete. Restart Klipper."
...
# Symlink Python backend
echo "[INSTALL] Linking filament_feed.py..."
