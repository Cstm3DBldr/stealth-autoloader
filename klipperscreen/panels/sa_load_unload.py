...
try:
    import sa_color_swatch as _swatch
except ImportError:
    _swatch = None
    logger.warning("sa_load_unload: could not import sa_color_swatch")
...
class Panel(ScreenPanel):
    ...
    def _populate_path_page(self):
        ...
        for i in range(num):
            ...
            hex2       = self._path_hex2s[i]        if i < len(self._path_hex2s)        else ''
            hex3       = self._path_hex3s[i]        if i < len(self._path_hex3s)        else ''
            mat        = self._path_mats[i]         if i < len(self._path_mats)         else ''

            btn = self._make_path_btn(i, state, hex_c, mat, color_type, hex2, hex3)
    ...
    def _make_path_btn(self, i, state, hex_c, mat,
        ...
        if _swatch is not None:
            if hex_c:
                ...
                swatch = _swatch.make_swatch_da(sw, hexes, color_type or 'single')
            else:
        ...
        row.pack_start(mat_lbl, True,  True,  0)
        btn.add(row)
    ...
    def _select_color(self, widget, c):
        ...
        self._wz['color_hex2']  = c.get('hex_2',       '')
        self._wz['color_hex3']  = c.get('hex_3',       '')
        page = self._pages['color']
    ...
    def _set_material_gcode(self):
        ...
        return (
            ...
            'COLOR_TYPE="{ctype}" COLOR_HEX_2="{chex2}" COLOR_HEX_3="{chex3}" '
            ...
            'PURGE_SPEED={ps} PURGE_LENGTH={pl}'.format(
                ...
                ctype=wz.get('color_type', 'single'),
                chex2=wz.get('color_hex2', ''),
                chex3=wz.get('color_hex3', ''),
    ...
    def _save_profile_and_return(self, show_msg=True):
        ...
        if i < len(self._path_color_types):
            self._path_color_types[i] = self._wz.get('color_type', 'single')
        if i < len(self._path_hex2s):
            self._path_hex2s[i] = self._wz.get('color_hex2', '')
        if i < len(self._path_hex3s):
            self._path_hex3s[i] = self._wz.get('color_hex3', '')
        self._wz = {}
    ...
    def _apply_sa(self, sa):
        ...
        self._path_color_types = sa.get("path_color_types",  ['single'] * num)
        self._path_hex2s       = sa.get("path_color_hex2s",  [''] * num)
        self._path_hex3s       = sa.get("path_color_hex3s",  [''] * num)
    ...
    def process_update(self, action, data):
        ...
        new_ctypes = sa.get("path_color_types",   self._path_color_types)
        new_hex2s  = sa.get("path_color_hex2s",   self._path_hex2s)
        new_hex3s  = sa.get("path_color_hex3s",   self._path_hex3s)
