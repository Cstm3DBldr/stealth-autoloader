...
try:
    ...
    _swatch = None
    logger.warning("sa_main: could not import sa_color_swatch")
...
class Panel(ScreenPanel):
    ...
    def _build_rows(self, num_paths):
        ...
        for i in range(num_paths):
            ...
            sw = 26
            da = Gtk.DrawingArea()
            da.set_size_request(sw, sw)
            da.set_valign(Gtk.Align.CENTER)
            # Mutable state dict — updated in _apply_sa, read in draw callback
            da._sa = {'hex': '', 'hex2': '', 'hex3': '',
                      'color_type': 'single', 'state': 'unknown'}

            def _make_draw_cb(widget):
                def _cb(w, cr):
                    ...
                    if _swatch:
                        if s['hex']:
                            hexes = [s['hex']]
                            if s['hex2']: hexes.append(s['hex2'])
                            if s['hex3']: hexes.append(s['hex3'])
                            _swatch.draw_swatch(cr, cx, cy, r, hexes, s['color_type'])
                        else:
                            _swatch.draw_state_swatch(cr, cx, cy, r, s['state'])
                    return False
                return _cb

            da.connect("draw", _make_draw_cb(da))

            color_name = Gtk.Label(label="---", halign=Gtk.Align.START,
                                   xalign=0.0, max_width_chars=20,
                                   ellipsize=3)
            color_box.pack_start(da,         False, False, 0)
            color_box.pack_start(color_name, False, False, 0)
            self._grid.attach(color_box, 7, row, 1, 1)
            self.labels['row_%d_swatch' % i] = da
    ...
    def _apply_sa(self, sa):
        ...
        ctypes    = sa.get("path_color_types",  [])
        hex2s     = sa.get("path_color_hex2s",  [])
        hex3s     = sa.get("path_color_hex3s",  [])
        ...
        for i in range(self._num_paths):
            ...
            if swatch and hasattr(swatch, '_sa'):
                swatch._sa['hex']        = hex_c
                swatch._sa['hex2']       = hex2s[i]  if i < len(hex2s)  else ''
                swatch._sa['hex3']       = hex3s[i]  if i < len(hex3s)  else ''
                swatch._sa['color_type'] = ctypes[i] if i < len(ctypes) else 'single'
                swatch._sa['state']      = state
                swatch.queue_draw()
            if color_lbl:
